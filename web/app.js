...
function renderScans() {
  ...
  for (const scan of state.overview.scans.slice(0, 7)) {
    const row = el('button', 'scan-row'); row.type = 'button'; const left = el('div'); left.append(el('div', 'scan-name', labels[scan.scenario]), el('div', 'scan-meta', `${scan.runtime === 'wasmer' ? 'Wasmer sandbox' : 'Simulation'} / ${scan.assessor === 'demo' ? 'Demo assessor' : 'Gemini'}`));
...
$('assessor').addEventListener('change', () => {$('mode-note').textContent = $('assessor').value === 'demo' ? 'The demo assessor is deterministic and is not an LLM security judgment.' : 'Gemini receives redacted synthetic test evidence and decides whether an alert is warranted.';});
