...
is visible in scan detail and every alert. The Gemini adapter is implemented
against the public generateContent endpoint with structured JSON; mocked API tests
pass. Live output quality, quota and provider compatibility still need a real key.
...
- Gemini: set GEMINI_API_KEY, retain/configure GEMINI_MODEL, restart, then select
...
The local .env and wasmer-access-token.txt are ignored by Git. Never place tokens
